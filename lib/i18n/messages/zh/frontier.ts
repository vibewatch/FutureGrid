export const frontierZh = {
  ...
  // ── 重构后的榜单（行即条形）───────────────────────────────────────────────
  /** 中性排序位次列/标签。并非能力排名——详见 dataDisclaimer。 */
  leadersColRank: "位次",
  /** 兼作可视化的榜单表格的屏幕阅读器标题。 */
  leadersTableCaption:
    "所选视图与指标下的追踪模型活动榜单。每一行列出实体、编码其在所选指标下数值的填充条、以文字显示的该数值，以及其峰值已报告训练算力。",
  /** 展开式披露区，收纳覆盖率与定义类注意事项。 */
  leadersWhyDisclosure: "这些数字意味着什么？",
  ...
  a11yFrontierLeadersSummary:
    "一个以榜单表格呈现的各机构与国家追踪模型活动。" +
    "每一行由实体名称、编码其在所选指标下数值的动态填充条，以及以文字显示的该数值与其峰值已报告训练算力组成。" +
    "机构标签页：前 12 个实体。国家标签页：前 10 个，默认按近三年追踪发布数量排序。" +
    "可通过上方控件切换指标；填充条与文字数值即为无障碍数据——不再单独提供图表或重复的数据表。",
  a11yFrontierLeadersName: "各机构与国家的追踪模型活动",
};
