...
| Gate | File |
|---|---|
| i18n key parity (careers) | `tests/careers-i18n.test.ts` — verifies en/zh key alignment for the `careers` namespace |
| i18n key parity (sectors) | `tests/sectors-i18n.test.ts` — asserts identical sorted key sets for the `sectors` namespace and no empty-string values in either locale |
...
| `tests/careers-i18n.test.ts` | i18n key-parity test (careers namespace) |
| `tests/sectors-i18n.test.ts` | i18n key-parity test (sectors namespace) |
